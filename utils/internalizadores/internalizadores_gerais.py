...
def internaliza_valores_venda(value):
    if not value:
        raise fieldError(msg='O campo não pode ser nulo')
    if not isinstance(value, Dict):
        raise fieldError(msg='O campo deve ser um dicionário com str : float ou JSON')
    chaves_invalidas = []
    valores_invalidos = []
    for chave, valor in value.items():
        if not isinstance(chave, str):
            chaves_invalidas.append(chave)
        if not isinstance(valor, float | int | Dict):
            valores_invalidos.append(valor)

    if chaves_invalidas or valores_invalidos:
        raise fieldError(msg=f'As seguintes chaves {chaves_invalidas} devem ser string, e seguinte valores {valores_invalidos} devem ser float ou JSON')
...
def internaliza_dicionario_str_str(value, esquema=None):
    if esquema is None:
        esquema = []
...
def internaliza_data(value, esquema=None):
    if esquema is None:
        esquema = []
    return value
...
def internaliza_string(string: Union[str, None], max_length: int, min_length: int, separador: str = "") -> str:
    if not string:
...
def internaliza_chave_estrangeira(primary_keys:list[int] | int, model:Type[models.Model]):
    if not primary_keys:
...
def internaliza_booleano(value):
    if not isinstance(value, bool):
        raise fieldError(msg="O valor deve ser Boolean. True ou False",value=value)
    return value

def internaliza_opcoes(value, option_tuples:set[tuple]):
    if not value:
...
def internaliza_lista_de_dicionarios(lista, field:str, field_double_check:str=""):
    if not lista:
...
def internaliza_conteudo_nucleos_particoes(nucleos:models.QuerySet, particoes:models.QuerySet):
