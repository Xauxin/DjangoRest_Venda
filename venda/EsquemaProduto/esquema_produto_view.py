...
from .esquema_produto_serializer import EsquemaProdutoSerializer,  EsquemaProduto
...
class EsquemaProdutoViewSet(viewsets.ModelViewSet):
    ...
    def retrieve(self, request, *args, **kwargs):
        ...
        try:
            id_esquema = kwargs['pk']
            modelo = EsquemaProduto.objects.get(id = id_esquema)
            modelo = serializer.detailed_to_representation(modelo)
        except EsquemaProduto.DoesNotExist as e:
    ...
    def create(self, request, *args, **kwargs):
        ...
        return Response(serializer.to_representation(objeto) , status=status.HTTP_201_CREATED, headers=headers)
            

    def update(self, request, *args, **kwargs):
        try:
            esquema = EsquemaProduto.objects.get(id = kwargs['pk'])
            serializer = self.get_serializer(data=request.data)
            serializer.is_valid()
